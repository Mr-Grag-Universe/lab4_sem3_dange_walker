...
Map::Map(const World & w) : hero(w.get_hero()), W(w.get_W()), H(w.get_H()) {
    ...
    const std::pair<unsigned int, unsigned int> h_p = hero.get_position();
    ...
    for (size_t i = 0, l = eff.size(); i < l; ++i) {
        std::pair <unsigned int, unsigned int> p = eff[i]->get_position();
        if (distance(p, h_p) <= 1900) {
            const sf::Vector2f hero_pos = sf::Vector2f((float) h_p.first, (float) h_p.second);
            ...
            std::cout << "hero: (" << hero_pos.x << "; " << hero_pos.y << "); ";
            // std::cout << "map: (" << W << "; " << H << "); ";
            std::cout << "s_w effect: (" << s_window_pos.x << "; " << s_window_pos.y << ");\n";
        }
    }
}